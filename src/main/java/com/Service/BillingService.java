...
public class BillingService {
	...
	public void billingRecordForSub(List<BillingEntity> entity) {

		try {
			hitBilling(entity);

//			String status = "0";
//			List<BillingEntity> getType = billingRepo.findByStatus(status);
//			for (BillingEntity getTypeEntity : getType) {
//				System.out.println("Hit Billing for Sub Request");
////				System.out.println("The value of getType is"+getTypeEntity.getType());
//				if(getTypeEntity.getType()==null || getTypeEntity.getType().equalsIgnoreCase("Sub")) {	
//					
//					if(getTypeEntity.getType()==null)
//					{
//						getTypeEntity.setType("Sub");
//				
//						billingRepo.save(getTypeEntity);
//					}
//					System.out.println("the value of type is"+getTypeEntity.getType());
//				List<BillingEntity> billingEntity = billingRepo.findByStatusAndType(status,getTypeEntity.getType());
//				for (BillingEntity entity : billingEntity) {
////					if (entity.getType().equalsIgnoreCase("sub")) {
//						hitBilling(billingEntity);
////						System.out.println("control inside hit billing for sub");
////					}
//					
//				}
//				
//				}
//				else if(getTypeEntity.getType()==null || getTypeEntity.getType().equalsIgnoreCase("REN"))
//				{
//					List<BillingEntity> billingEntity = billingRepo.findByStatusAndType(status,getTypeEntity.getType());
//					for (BillingEntity entity : billingEntity) {
////						if (entity.getType().equalsIgnoreCase("sub")) {
...
//			}
	...
	public void hitBilling(List<BillingEntity> list) {
...
//			String url = "http://43.250.139.33:10381/TisService";
		...
		for (BillingEntity entity : list) {
			List<TblDnd> dnd = dndRepo.findByAni(String.valueOf(entity.getAni()));
...
//				System.out.println("The vaulue of dnd is"+tblDnd);
			if (dnd.size() == 0) {
				...
				if(String.valueOf(entity.getAni()).equals("93700748335") || String.valueOf(entity.getAni()).equals("93701515994")) {
					
				
//				List<Tbl_Billing_Model> billing_Model = billingRepo.getCountAni(String.valueOf(entity.getAni()));	

//				if(billing_Model.equals("1")) {
				String ani = String.valueOf(entity.getAni());
...
//									billingRepo.save(entity);
									...
									saveRecordInTableLogs(entity, lastBilledDate, nextBillingDate, request.toString(),
											body + " " + status);
									saveInTableBillingSuccess(entity, request.toString(), body + " " + status);
										...
										&& entity.getBillingType().equalsIgnoreCase("auto")) {
...
//								System.out.println("control inside else block of daily pack");
								...
								} else if (entity.getType().equalsIgnoreCase("Sub")
										&& entity.getBillingType().equalsIgnoreCase("one")) {
									service.sendSms(entity);
									updateDataInSub(subscription, entity, lastBilledDate, nextBillingDate);
									saveInTableBillingSuccess(entity, request.toString(), body + " " + status);
								}
...
//					e.printStackTrace();
				...
				try {
					TblSubscription subscription = subscriptionRepository.getAni(String.valueOf(entity.getAni()));
...
//			billingLogs.setProcessDate(entity.getProcessDate());
			...
			if (entity.getStatus().equalsIgnoreCase("Failed")) {
				if (entity.getType().equalsIgnoreCase("sub") && entity.getBillingType().equalsIgnoreCase("auto")) {
					billingLogs.setFailedStatus("0");
					logsRepo.save(billingLogs);
				} 
				else if(entity.getType().equalsIgnoreCase("sub") && entity.getBillingType().equalsIgnoreCase("one") )
				{
					billingLogs.setFailedStatus("OneTime");
					logsRepo.save(billingLogs);				
				}
				else {
...
//				billingRepo.delete(entity);
	...
	public void saveInTableBillingSuccess(BillingEntity success, String request, String response) {
		try {
			...
			billingSuccess.setAni(String.valueOf(success.getAni()));
	...
	public void saveInSub(BillingEntity entity, LocalDateTime lastBilledDate, LocalDateTime nextBillingDate) {
		try {
			...
			if (entity.getPackType().equalsIgnoreCase("Daily")) {
				...
				newDate = LocalDate.now().plusDays(7);
			} else if (entity.getPackType().equalsIgnoreCase("monthly")) {
			...
			newRecord.setAni(String.valueOf(entity.getAni()));
